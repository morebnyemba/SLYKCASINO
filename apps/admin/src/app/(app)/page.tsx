  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Online players" value={loading ? '…' : (stats?.online_players ?? '—')} hint="realtime" />
        <StatCard label="Open bets" value={loading ? '…' : (stats?.open_bets ?? '—')} />
        <StatCard label="Active promos" value={loading ? '…' : (stats?.active_promotions ?? '—')} />
        <StatCard label="Open chats" value={loading ? '…' : (stats?.open_chats ?? '—')} />
      </div>
